...
const formatDecimal = (value: number | string | null | undefined, digits = 2) => Number(value || 0).toFixed(digits);
const formatDateTime = (value: string | null | undefined) => {
  if (!value) return "N/A";
  const parsed = new Date(value);
  return Number.isNaN(parsed.getTime()) ? "N/A" : parsed.toLocaleString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
};
...
const sorterPalette = ["#1565c0", "#2e7d32", "#ef6c00", "#6a1b9a", "#00838f", "#c62828", "#ad1457", "#5d4037", "#0277bd", "#7b1fa2"];
...
const MetricCard = ({ label, value, color, helper }: { label: string; value: string | number; color: string; helper?: string }) => (
  <Card sx={{ borderRadius: 2, border: "1px solid #e3e8ef", height: "100%" }}>
    <CardContent>
      <Typography variant="caption" color="text.secondary">{label}</Typography>
      <Typography variant="h5" sx={{ mt: 0.6, color, fontWeight: 700 }}>{typeof value === "number" ? formatNumber(value) : value}</Typography>
...
const HorizontalBarList = ({ items, labelKey, valueKey, valueFormatter, color = "#1565c0", maxItems = 10 }: { items: Record<string, any>[]; labelKey: string; valueKey: string; valueFormatter?: (value: number) => string; color?: string; maxItems?: number; }) => {
  ...
  return <Stack spacing={1.1}>{rows.map((row, index) => { const value = Number(row[valueKey] || 0); const widthPct = maxValue > 0 ? Math.max((value / maxValue) * 100, 2) : 0; return <Box key={`${row[labelKey]}-${index}`}><Box display="flex" justifyContent="space-between" mb={0.35} gap={1}><Typography variant="body2" sx={{ fontWeight: 600 }}>{row[labelKey] || "N/A"}</Typography><Typography variant="body2" color="text.secondary">{valueFormatter ? valueFormatter(value) : formatNumber(value)}</Typography></Box><Box sx={{ height: 9, borderRadius: 999, bgcolor: "#edf1f7", overflow: "hidden" }}><Box sx={{ width: `${widthPct}%`, height: "100%", borderRadius: 999, bgcolor: color }} /></Box></Box>; })}</Stack>;
};
const DonutShareChart = ({ title, items, totalLabel, formatter }: { title: string; items: { label: string; value: number; color: string }[]; totalLabel: string; formatter?: (value: number) => string; }) => {
  const filtered = items.filter((item) => item.value > 0);
  const total = filtered.reduce((sum, item) => sum + item.value, 0);
  const radius = 44;
  const circumference = 2 * Math.PI * radius;
  let progress = 0;
  return <Paper sx={{ p: 2, borderRadius: 2, height: "100%" }}><Typography variant="h6" sx={{ mb: 1.5 }}>{title}</Typography>{filtered.length === 0 || total <= 0 ? <Typography variant="body2" color="text.secondary">No data available for this range.</Typography> : <Grid container spacing={2} alignItems="center"><Grid item xs={12} md={5}><Box sx={{ display: "flex", justifyContent: "center" }}><Box sx={{ position: "relative", width: 128, height: 128 }}><svg width="128" height="128" viewBox="0 0 128 128"><circle cx="64" cy="64" r={radius} fill="none" stroke="#eef2f7" strokeWidth="18" /><g transform="rotate(-90 64 64)">{filtered.map((item) => { const segment = (item.value / total) * circumference; const dashOffset = circumference - progress; progress += segment; return <circle key={item.label} cx="64" cy="64" r={radius} fill="none" stroke={item.color} strokeWidth="18" strokeDasharray={`${segment} ${circumference - segment}`} strokeDashoffset={dashOffset} strokeLinecap="butt" />; })}</g></svg><Box sx={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}><Typography variant="h6" sx={{ fontWeight: 700 }}>{formatter ? formatter(total) : formatNumber(total)}</Typography><Typography variant="caption" color="text.secondary">{totalLabel}</Typography></Box></Box></Box></Grid><Grid item xs={12} md={7}><Stack spacing={1}>{filtered.map((item) => { const pct = total > 0 ? (item.value / total) * 100 : 0; return <Box key={item.label}><Box display="flex" justifyContent="space-between" gap={1}><Stack direction="row" spacing={1} alignItems="center"><Box sx={{ width: 12, height: 12, borderRadius: "50%", bgcolor: item.color }} /><Typography variant="body2" sx={{ fontWeight: 600 }}>{item.label}</Typography></Stack><Typography variant="body2" color="text.secondary">{formatter ? formatter(item.value) : formatNumber(item.value)} ({formatPct(pct)})</Typography></Box></Box>; })}</Stack></Grid></Grid>}</Paper>;
};

const StackedShowBars = ({ title, rows, valueKey, valueLabel, sorterColors }: { title: string; rows: SortingShowSorterRow[]; valueKey: "shipmentsClosed" | "totalUnits" | "closeRelevantUnits"; valueLabel: string; sorterColors: Record<string, string>; }) => {
  const grouped = Object.values(rows.reduce<Record<string, { showName: string; total: number; segments: { sorterName: string; value: number; color: string }[] }>>((acc, row) => {
    const key = `${row.showId}-${row.showName}`;
    if (!acc[key]) acc[key] = { showName: row.showName, total: 0, segments: [] };
    const value = Number(row[valueKey] || 0);
    acc[key].total += value;
    acc[key].segments.push({ sorterName: row.sorterName, value, color: sorterColors[row.sorterName] || "#94a3b8" });
    return acc;
  }, {})).sort((a, b) => b.total - a.total);
  return <Paper sx={{ p: 2, borderRadius: 2, height: "100%" }}><Typography variant="h6" sx={{ mb: 1.5 }}>{title}</Typography>{grouped.length === 0 ? <Typography variant="body2" color="text.secondary">No show-level data available for this range.</Typography> : <Stack spacing={1.5}>{grouped.map((show) => <Box key={`${show.showName}-${valueKey}`}><Box display="flex" justifyContent="space-between" gap={1} mb={0.4}><Typography variant="body2" sx={{ fontWeight: 600 }}>{show.showName}</Typography><Typography variant="body2" color="text.secondary">{formatNumber(show.total)} {valueLabel}</Typography></Box><Box sx={{ display: "flex", height: 24, borderRadius: 999, overflow: "hidden", bgcolor: "#edf1f7", border: "1px solid #e4ebf3" }}>{show.segments.filter((segment) => segment.value > 0).map((segment) => { const pct = show.total > 0 ? Math.max((segment.value / show.total) * 100, 4) : 0; return <Box key={`${show.showName}-${segment.sorterName}`} title={`${show.showName} - ${segment.sorterName}: ${formatNumber(segment.value)} ${valueLabel}`} sx={{ width: `${pct}%`, bgcolor: segment.color, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 11, fontWeight: 700 }}>{pct > 14 ? formatNumber(segment.value) : ""}</Box>; })}</Box></Box>)}</Stack>}</Paper>;
};
...
function SortingAnalytics() {
  ...
  const averageHandlingMinutes = useMemo(() => { const valid = scanTimingRows.filter((row) => row.avgHandlingMinutes !== null && row.avgHandlingMinutes !== undefined); if (valid.length === 0) return null; return valid.reduce((sum, row) => sum + Number(row.avgHandlingMinutes || 0), 0) / valid.length; }, [scanTimingRows]);
    ...
    const fetchAll = async () => {
      try {
        ...
        const results = await Promise.allSettled([
          ...
          axios.get(getApiUrl("whatnot/analytics/fulfillment-sorting-sorters-daily"), { params: { ...params, limit: 10 } }),
        ...
        setError("Failed to load sorting analytics. Please refresh and try again.");
  ...
  const overviewCards = [
    ...
    { label: "Active Sorters", value: overview?.activeSorters || 0, color: "#6a1b9a", helper: `${formatDecimal(overview?.avgShipmentsPerSorter || 0)} shipments/sorter` },
    { label: "Avg Units / Shipment", value: formatDecimal(overview?.avgUnitsPerShipment || 0), color: "#ef6c00", helper: `${formatDecimal(overview?.avgCloseRelevantUnitsPerShipment || 0)} close-relevant` },
    { label: "Avg Scan Gap", value: averageScanGap !== null ? formatDurationSeconds(averageScanGap) : "N/A", color: "#00838f", helper: averageHandlingMinutes !== null ? `${formatDurationMinutes(averageHandlingMinutes)} avg handling time` : "Waiting for scan timing data" },
